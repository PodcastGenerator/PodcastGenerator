			...
			if ($fileData != NULL) { //This IF avoids notice error in PHP4 of undefined variable $fileData[0]
				...
				if ($file_multimediale[1]=="$podcast_filetype") { // if the extension is the same as specified in config.php
					...
					if (file_exists("$filedescr")) { //if database file exists 
						...
						$PG_mainbody .= 
							'<div class="episode">
							<p><b>'.$text_title.'</b><span class="alert">'.$episode_date.'</span></p><p>[<a href="?p=episode&amp;name='.$file_multimediale[0].'.'.$podcast_filetype.'">'._("Show").'</a> - <a href="?p=admin&do=edit&amp;name='.$file_multimediale[0].'.'.$podcast_filetype.'">'._("Edit").'</a> - <a href="javascript:Effect.toggle(\''.$text_title2.$recent_count.'\',\'appear\');">'._("Delete").'</a>]</p>
