...
for ($i = 0; $i < sizeof($episodes); $i++) {
    ...
    echo '  <h1><a href="index.php?' . $link . '=' . htmlspecialchars($episodes[$i]["episode"]["filename"]) . '">' . htmlspecialchars($episodes[$i]["episode"]["titlePG"]) . '</a></h1>';             // Headline
    ...
    if (isset($_SESSION["username"])) {
        echo '  <a class="btn btn-dark btn-sm" href="admin/episodes_edit.php' . $link . htmlspecialchars($episodes[$i]["episode"]["filename"]) . '">Edit/Delete (Admin)</a>';
    }
    echo '  <a class="btn btn-outline-primary btn-sm" href="' . $link . htmlspecialchars($episodes[$i]["episode"]["filename"]) . '">More</a>
