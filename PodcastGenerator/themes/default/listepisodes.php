...
for ($i = 0; $i < sizeof($episode_chunk); $i++) {
    ...
    echo '  <h1><a href="' . $config['indexfile'] . '?' . $link . '=' . $item[$i]["episode"]["filename"] . '">' . htmlspecialchars($item[$i]["episode"]["titlePG"]) . '</a></h1>';             // Headline
    ...
    echo '  <a class="btn btn-outline-primary btn-sm" href="' . $config['indexfile'] . '?' . $link . '=' . $item[$i]["episode"]["filename"] . '">' . $more . '</a>
                <a class="btn btn-outline-success btn-sm" href="media/' . $item[$i]["episode"]["filename"] . '">' . $download . '</a><br>';                      // Buttons
    if ($type != 'invalid') {
