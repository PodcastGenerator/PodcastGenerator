...
if (isset($no_episodes)) {
    echo '<div class="col-lg-6 text-white"><p>' . $no_episodes . '</p></div>';
} else {
    for ($i = 0; $i < sizeof($episode_chunk); $i++) {
        ...
        echo '  <h5><a class="text-white" href="' . $config['indexfile'] . '?' . $link . '=' . $item[$i]["episode"]["filename"] . '">' . $item[$i]["episode"]["titlePG"] . '</a></h5>';
        echo '  <p><i class="fa fa-calendar" aria-hidden="true"></i> <small class="text-white">' . $item[$i]["episode"]["moddate"] . '</small></p> ';
        ...
        echo '  <a class="btn btn-outline-light btn-sm" href="' . $config['indexfile'] . '?' . $link . '=' . $item[$i]["episode"]["filename"] . '">' . $more . '</a>
                <a class="btn btn-outline-light btn-sm" href="media/' . $item[$i]["episode"]["filename"] . '">' . $download . '</a><br>';
        if ($type != 'invalid') {
            echo '  <small style="font-size:65%" class="text-white">' . $filetype . ': ' . strtoupper(pathinfo($config["upload_dir"] . $item[$i]["episode"]["filename"], PATHINFO_EXTENSION)) . '
